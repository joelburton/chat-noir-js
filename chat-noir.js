...
class Board {
  ...
  gameEnd(outcome) {
    // We want the DOM to update to show the end board before the alert pops up,
    // so do this in a setTimeout so it happens after the DOM update.
    window.setTimeout(() => alert("You " + outcome), 0);
  }
}
